...
/**
 * Performance test for the PaymentChannel entity.
 */
class PaymentChannelGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PaymentChannel entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all paymentChannels")
            .get("/api/payment-channels")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new paymentChannel")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_paymentChannel_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created paymentChannel")
                .get("${new_paymentChannel_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created paymentChannel")
            .delete("${new_paymentChannel_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
