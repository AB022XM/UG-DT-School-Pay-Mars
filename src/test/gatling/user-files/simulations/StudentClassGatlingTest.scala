...
/**
 * Performance test for the StudentClass entity.
 */
class StudentClassGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the StudentClass entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all studentClasses")
            .get("/api/student-classes")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new studentClass")
            .post("/api/student-classes")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "recordUniqueIdentifier":null
                , "studentClassId":"0"
                , "studentClassCode":"SAMPLE_TEXT"
                , "studentClassName":"SAMPLE_TEXT"
                , "studentClassDescription":"SAMPLE_TEXT"
                , "status":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_studentClass_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created studentClass")
                .get("${new_studentClass_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created studentClass")
            .delete("${new_studentClass_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
