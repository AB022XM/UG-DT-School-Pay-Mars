...
type RestOf<T extends IPayment | NewPayment> = Omit<
  T,
  'processTimestamp' | 'feeDueFromDate' | 'feeDueToDate' | 'dateOfBirth' | 'createdAt' | 'updatedAt'
> & {
...
export class PaymentService {
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...payment,
      processTimestamp: payment.processTimestamp?.format(DATE_FORMAT) ?? null,
      feeDueFromDate: payment.feeDueFromDate?.format(DATE_FORMAT) ?? null,
      feeDueToDate: payment.feeDueToDate?.format(DATE_FORMAT) ?? null,
      dateOfBirth: payment.dateOfBirth?.format(DATE_FORMAT) ?? null,
      createdAt: payment.createdAt?.format(DATE_FORMAT) ?? null,
      updatedAt: payment.updatedAt?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...restPayment,
      processTimestamp: restPayment.processTimestamp ? dayjs(restPayment.processTimestamp) : undefined,
      feeDueFromDate: restPayment.feeDueFromDate ? dayjs(restPayment.feeDueFromDate) : undefined,
      feeDueToDate: restPayment.feeDueToDate ? dayjs(restPayment.feeDueToDate) : undefined,
      dateOfBirth: restPayment.dateOfBirth ? dayjs(restPayment.dateOfBirth) : undefined,
