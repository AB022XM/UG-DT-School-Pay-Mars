...
type RestOf<T extends ISchool | NewSchool> = Omit<T, 'createdAt' | 'updatedAt' | 'isDeleted'> & {
...
export class SchoolService {
  ...
  protected convertDateFromClient<T extends ISchool | NewSchool | PartialUpdateSchool>(school: T): RestOf<T> {
    return {
      ...school,
      createdAt: school.createdAt?.format(DATE_FORMAT) ?? null,
      updatedAt: school.updatedAt?.format(DATE_FORMAT) ?? null,
      isDeleted: school.isDeleted?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSchool: RestSchool): ISchool {
    return {
      ...restSchool,
      createdAt: restSchool.createdAt ? dayjs(restSchool.createdAt) : undefined,
      updatedAt: restSchool.updatedAt ? dayjs(restSchool.updatedAt) : undefined,
      isDeleted: restSchool.isDeleted ? dayjs(restSchool.isDeleted) : undefined,
    };
  }
